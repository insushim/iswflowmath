...
export default function AchievementsPage() {
  ...
  const loadUserStats = async (uid: string) => {
    try {
      ...
      const userData = userResult?.user || null;
      ...
      try {
        sessions = await getUserSessions(uid, 1000);
      } catch (e) {
        console.error("Failed to load sessions:", e);
      }
      const totalProblems = sessions.reduce(
        (sum, s) => sum + (s.totalProblems || s.problemsAttempted || 0),
        0,
      );
      const totalCorrect = sessions.reduce(
        (sum, s) => sum + (s.correctAnswers || s.problemsCorrect || 0),
        0,
      );
      ...
      setUserStats({
        ...
        streakDays: userData?.streak_days || userData?.streakDays || 0,
        totalStudyTime,
        averageAccuracy:
  ...
  const achievements = [
    {
      ...
      requirement: 1,
      xpReward: 10,
      check: (s: UserStats) => s.totalProblems >= 1,
      getProgress: (s: UserStats) => Math.min(s.totalProblems, 1),
    },
    {
      id: "problem-10",
      name: "꾸준한 학습자",
      ...
      requirement: 10,
      xpReward: 50,
      check: (s: UserStats) => s.totalProblems >= 10,
      getProgress: (s: UserStats) => Math.min(s.totalProblems, 10),
    },
    {
      id: "problem-50",
      name: "열정적인 학습자",
      ...
      requirement: 50,
      xpReward: 100,
      check: (s: UserStats) => s.totalProblems >= 50,
      getProgress: (s: UserStats) => Math.min(s.totalProblems, 50),
    },
    {
      id: "problem-100",
      name: "문제 정복자",
      ...
      requirement: 100,
      xpReward: 200,
      check: (s: UserStats) => s.totalProblems >= 100,
      getProgress: (s: UserStats) => Math.min(s.totalProblems, 100),
    },
    {
      id: "problem-500",
      name: "수학 마스터",
      ...
      requirement: 500,
      xpReward: 500,
      check: (s: UserStats) => s.totalProblems >= 500,
      getProgress: (s: UserStats) => Math.min(s.totalProblems, 500),
    },
    {
      id: "streak-3",
      name: "3일 연속",
      ...
      requirement: 3,
      xpReward: 30,
      check: (s: UserStats) => s.streakDays >= 3,
      getProgress: (s: UserStats) => Math.min(s.streakDays, 3),
    },
    {
      id: "streak-7",
      name: "일주일 챔피언",
      ...
      requirement: 7,
      xpReward: 70,
      check: (s: UserStats) => s.streakDays >= 7,
      getProgress: (s: UserStats) => Math.min(s.streakDays, 7),
    },
    {
      id: "streak-30",
      name: "한 달의 기적",
      ...
      requirement: 30,
      xpReward: 300,
      check: (s: UserStats) => s.streakDays >= 30,
      getProgress: (s: UserStats) => Math.min(s.streakDays, 30),
    },
    {
      id: "accuracy-80",
      name: "정확한 계산",
      description: "평균 정확도 80% 달성",
      category: "accuracy",
      requirement: 80,
      xpReward: 50,
      check: (s: UserStats) => s.averageAccuracy >= 80,
      getProgress: (s: UserStats) =>
        Math.min(Math.round(s.averageAccuracy), 80),
    },
    {
      id: "accuracy-90",
      name: "거의 완벽",
      description: "평균 정확도 90% 달성",
      category: "accuracy",
      requirement: 90,
      xpReward: 100,
      check: (s: UserStats) => s.averageAccuracy >= 90,
      getProgress: (s: UserStats) =>
        Math.min(Math.round(s.averageAccuracy), 90),
    },
    {
      id: "perfect-session",
      name: "완벽한 세션",
      ...
      requirement: 100,
      xpReward: 200,
      check: (s: UserStats) => s.bestAccuracy >= 100,
      getProgress: (s: UserStats) => Math.min(Math.round(s.bestAccuracy), 100),
    },
    {
      id: "flow-10",
      name: "몰입의 시작",
      description: "총 학습 시간 10분 달성",
      category: "flow",
      requirement: 10,
      xpReward: 50,
      check: (s: UserStats) => s.totalStudyTime >= 10,
      getProgress: (s: UserStats) => Math.min(s.totalStudyTime, 10),
    },
    {
      id: "flow-60",
      name: "깊은 몰입",
      ...
      requirement: 60,
      xpReward: 100,
      check: (s: UserStats) => s.totalStudyTime >= 60,
      getProgress: (s: UserStats) => Math.min(s.totalStudyTime, 60),
    },
    {
      id: "flow-300",
      name: "몰입 마스터",
      description: "총 학습 시간 5시간 달성",
      category: "flow",
      requirement: 300,
      xpReward: 200,
      check: (s: UserStats) => s.totalStudyTime >= 300,
      getProgress: (s: UserStats) => Math.min(s.totalStudyTime, 300),
    },
    {
      id: "quick-learner",
      name: "빠른 학습자",
      description: "10개 이상의 문제를 정답",
      category: "speed",
      requirement: 10,
      xpReward: 50,
      check: (s: UserStats) => s.totalCorrect >= 10,
      getProgress: (s: UserStats) => Math.min(s.totalCorrect, 10),
    },
    {
      id: "session-master",
      name: "세션 마스터",
      description: "10개 이상의 학습 세션 완료",
      category: "mastery",
      requirement: 10,
      xpReward: 200,
      check: (s: UserStats) => s.totalSessions >= 10,
      getProgress: (s: UserStats) => Math.min(s.totalSessions, 10),
    },
    {
      id: "dedicated-learner",
      name: "헌신적인 학습자",
      description: "50개 이상의 학습 세션 완료",
      category: "mastery",
      requirement: 50,
      xpReward: 500,
      check: (s: UserStats) => s.totalSessions >= 50,
      getProgress: (s: UserStats) => Math.min(s.totalSessions, 50),
    },
  ].map((def) => ({
    ...def,
    unlocked: def.check(userStats),
    progress: def.getProgress(userStats),
  }));
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <h3
                      className={`font-bold ${achievement.unlocked ? "text-white" : "text-slate-400"}`}
                    >
                      {achievement.name}
                    </h3>
                    <span
