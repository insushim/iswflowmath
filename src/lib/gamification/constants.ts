// ============================================================
// 셈마루(SemMaru) - Gamification Constants
// ============================================================
...
export function getXpForLevel(level: number): number {
  if (level <= 1) return 0;
  return Math.floor(
    LEVEL_CONFIG.BASE_XP * Math.pow(LEVEL_CONFIG.SCALING_FACTOR, level - 2),
  );
}
...
export function calculateLevel(totalXp: number): {
  ...
  const xpToNext =
    level < LEVEL_CONFIG.MAX_LEVEL ? getXpForLevel(level + 1) : 0;
  const progress =
    xpToNext > 0 ? Math.min(100, (xpRemaining / xpToNext) * 100) : 100;
