// ============================================================
// 셈마루(SemMaru) - 수준별 문제 생성을 위한 종합 참고 데이터
...
export type PISAContext =
  | "personal"
  | "occupational"
  | "societal"
  | "scientific";
export type PISAContent =
  | "quantity"
  | "space_shape"
...
export const PISA_CONTEXTS: Record<
  PISAContext,
  { korean: string; examples: string[] }
> = {
  personal: {
    korean: "개인적",
...
export function determineFlowZone(recentSuccessRate: number): FlowZoneConfig {
  for (const zone of FLOW_ZONES) {
    if (
      recentSuccessRate >= zone.successRateRange[0] &&
      recentSuccessRate < zone.successRateRange[1]
    ) {
      return zone;
    }
  }
...
export const COMMON_MISCONCEPTIONS: Misconception[] = [
  ...
  // 미적분 오개념
  {
    ...
    diagnosticQuestion:
      "f(2)가 정의되지 않아도 lim(x→2) f(x)가 존재할 수 있나요?",
  },
  {
    topic: "calculus",
    subtopic: "미분",
...
export function getMisconceptionsForTopic(
  topic: MathTopic,
  grade: number,
): Misconception[] {
  return COMMON_MISCONCEPTIONS.filter(
    (m) =>
      m.topic === topic && grade >= m.gradeRange[0] && grade <= m.gradeRange[1],
  );
}
...
export const PROBLEM_TYPES: ProblemTypeConfig[] = [
  ...
  },
  {
    ...
    difficultyBonus: 0.4,
    templateStructure:
      "{constraint} 조건에서 {objective}의 최댓값(최솟값)을 구하시오.",
  },
];
...
export const CCSS_FRAMEWORK: InternationalStandard = {
  ...
  benchmarks: [
    {
      level: "HS Advanced",
      description: "미적분 준비 완료",
      koreanEquivalent: 12,
    },
    {
      level: "HS Proficient",
      description: "고등 수학 이해",
      koreanEquivalent: 10,
    },
    {
      level: "MS Proficient",
      description: "중등 수학 숙달",
      koreanEquivalent: 8,
    },
    {
      level: "Elementary Proficient",
      description: "초등 수학 기초",
      koreanEquivalent: 5,
    },
  ],
};
...
export function getIRTParametersForGrade(
  grade: number,
): IRTParameterRange | null {
...
export function generateProblemContext(
  ...
  const curriculum = getCurriculumForGrade(grade).find(
    (c) => c.topic === topic,
  );
  ...
  const pisaProcess =
    PISA_PROCESSES[Math.floor(Math.random() * PISA_PROCESSES.length)];
